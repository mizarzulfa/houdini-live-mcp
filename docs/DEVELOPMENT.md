...
1. Restart Claude Desktop (it spawns the two servers), then enable both connectors in a chat: **+** (plus) button → Connectors → toggle **houdini-docs** and **houdini-live** on.
...
| Tool | Purpose |
|---|---|
| `capture_viewport(width, height, viewport=None)` | Single-frame viewport render returned as an image, so Claude can *see* the scene |
...
| `flipbook_frames(...)` | Pollable per-frame driver (`cursor` protocol) that keeps the bridge responsive for long ranges; encodes mp4 on the final call |
...
- **Deferred auto-start.** `uiready.py` starts the bridge on a deferred event-loop tick, not inline, because starting inline races hwebserver's own init and leaves a route-less server up.
